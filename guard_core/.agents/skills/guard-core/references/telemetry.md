...
This is enforced by construction, not by auditing every construction site. No module in `guard_core/` references `guard_agent` at runtime at all — not an import, not an aliased import, not a submodule import followed by attribute access — except `_pydantic_plugin_mute.py` itself and `models.py` (for `AgentConfig` alone, which is not a telemetry model). Every other construction site, including per-request paths in the event bus, the decorators, the detection engine, rate limiting, IP banning, dynamic rules, behavior tracking, Redis, cloud-provider blocking, geo-IP, and security headers, calls `guard_core._pydantic_plugin_mute.get_telemetry_model(name)`, which runs the mute and only then returns the class. `tests/test_telemetry_model_access.py` walks the AST of every file in `guard_core/` (the sync tree is generated from it, so it is the one source of truth) and fails the build if any module outside that two-file allowlist references `guard_agent` at runtime — keyed on the module being named, not on which symbol is imported, so an aliased import or a `getattr(guard_agent, "SecurityEvent")` indirection is caught exactly like a direct `from guard_agent import SecurityEvent`, and a `TYPE_CHECKING`-only reference is exempted since it never runs. That is what a new construction site cannot get past, not a re-audited list of call graphs.
