...
* `EVENT_TYPE_VALUES` (34): `access_denied`, `authentication_failed`, `behavior_violation`, `cloud_blocked`, `content_filtered`, `country_blocked`, `csp_violation`, `custom_request_check`, `decoding_error`, `decorator_violation`, `dynamic_rule_applied`, `dynamic_rule_updated`, `dynamic_rule_violation`, `emergency_mode_activated`, `emergency_mode_block`, `geo_lookup_failed`, `https_enforced`, `ip_ban_failed`, `ip_banned`, `ip_blocked`, `ip_unbanned`, `path_excluded`, `pattern_added`, `pattern_detected`, `pattern_removed`, `penetration_attempt`, `rate_limited`, `rate_limit_script_reloaded`, `redis_connection`, `redis_error`, `route_unresolved`, `security_bypass`, `security_headers_applied`, `user_agent_blocked`. `ip_ban_failed` (`EVENT_IP_BAN_FAILED`, `guard_core/core/events/event_types.py`) is emitted by `_emit_ban_escalation_failed` (`guard_core/core/checks/helpers.py`) when the auto-ban escalation's own `ban_ip` call raises; the loopback/trusted-proxy ban refusal does not raise, so it never triggers this event. `ban_ip` (`guard_core/handlers/_ipban_bans.py:146-159`) is still called in that case, it just returns early once `_self_dos_refusal_reason` names a refusal reason, after logging the refusal warning and before any ban is stored.
...
The IPInfo token no longer travels as a URL query parameter: `IPInfoManager._download_database` (`guard_core/handlers/ipinfo_handler.py`) sends it as an `Authorization: Bearer <token>` request header, so it cannot leak into a logged or telemetered URL. A download failure's reported reason is reduced to the exception's class name plus HTTP status when available (`_describe_download_error`), instead of the raw exception `str()`, which could otherwise carry the request URL.

`RedisManager.initialize` (`guard_core/handlers/redis_handler.py`) strips userinfo from `redis_url` via `_redact_redis_url` before it reaches the `redis_connection`/`redis_error` agent events: only `scheme://host[:port]` is sent, never embedded Redis credentials.
