...
Current as of guard-core 3.17.1.
...
* **A deeply-braced query param, header, or form value cannot crash embedded-JSON detection.** The embedded-JSON bridge `_check_embedded_json` (`guard_core/_utils/embedded_json_scan.py`) looks for a JSON object or array embedded inside a single non-body value and hands it to the body walker `_scan_json_value` (`guard_core/_utils/body_json_scan.py`); it catches `RecursionError` from `json.loads` the same way it catches `json.JSONDecodeError`, treats the value as not-embedded-JSON, and falls through to the ordinary raw-value pattern scan instead of letting the exception escape `detect_penetration_attempt`. This is the former `_check_json_fields`/`_try_check_json_value` pair (`guard_core/_utils/detection_scan.py`), replaced in 3.17.1 by the current bridge when the module split removed both with no alias.
