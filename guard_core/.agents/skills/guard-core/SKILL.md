...
* **Two previously-silent misconfigurations now warn at construction.** A `trusted_proxies` entry that is a `/0` network (`0.0.0.0/0`, `::/0`) trusts every peer to set `X-Forwarded-For`; an empty `enabled_detection_categories` with `enable_penetration_detection=True` runs detection that can never match anything. Both still succeed, but now log a `WARNING` on the `guard_core.models` logger (`guard_core/_security_config_validators.py`, wired into `SecurityConfig` in `guard_core/models.py`).
