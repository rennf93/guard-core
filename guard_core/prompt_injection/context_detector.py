...
class ContextAwareDetector:
    ...
    def is_suspicious_in_context(
        ...
        """
        if self._context_pattern_match(text, context_type):
            return True
        if self._is_suspicious_context_switch(context_type):
            return True
        if user_id and self._check_user_anomaly(user_id, text):
            return True
        self.context_history.append((text, context_type))
        return False

    def _context_pattern_match(self, text: str, context_type: ContextType) -> bool:
        if context_type == ContextType.RAG_QUERY:
            return self.check_rag_specific_patterns(text)
        if context_type == ContextType.CODE_GENERATION:
            return self.check_code_injection_patterns(text)
        if context_type in (ContextType.GENERAL, ContextType.DATA_ANALYSIS):
            return self.check_chat_patterns(text)
        return False

    def _is_suspicious_context_switch(self, context_type: ContextType) -> bool:
        if not self.context_history:
            return False
        return self.detect_context_switch(context_type, self.context_history[-1][1])

    def _check_user_anomaly(self, user_id: str, text: str) -> bool:
    ...
    def get_context_score(
        ...
        """
        score = self._context_pattern_score(text, context_type)
        score += self._context_switch_score(context_type)
        if user_id:
            score += self._user_profile_score(user_id, text)
        return min(1.0, score)

    def _context_pattern_score(self, text: str, context_type: ContextType) -> float:
        if context_type == ContextType.RAG_QUERY and self.check_rag_specific_patterns(
            text
        ):
            return 0.5
        if (
            context_type == ContextType.CODE_GENERATION
            and self.check_code_injection_patterns(text)
        ):
            return 0.7
        if context_type == ContextType.GENERAL and self.check_chat_patterns(text):
            return 0.6
        return 0.0

    def _context_switch_score(self, context_type: ContextType) -> float:
        if self._is_suspicious_context_switch(context_type):
            return 0.3
        return 0.0

    def _user_profile_score(self, user_id: str, text: str) -> float:
        profile = self.get_user_profile(user_id)
        ...
        if profile.avg_length > 0:
            length_ratio = abs(len(text) - profile.avg_length) / profile.avg_length
            if length_ratio > 2.0:
                score += 0.2
