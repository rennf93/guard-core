...
if TYPE_CHECKING:
    ...
    from guard_core.sync.prompt_injection.pattern_detector import (
        PatternDetector,
    )
...
class InjectionScorer:
    ...
    def _check_semantic_layers(self, text: str) -> list[str]:
        ...
        if normalized and normalized != text.lower():
            if self.pattern_detector.is_suspicious(normalized):
                norm = self.pattern_detector.get_matched_patterns(normalized)
    ...
    def score(self, text: str) -> InjectionScore:
        ...
        if self.pattern_detector.is_suspicious(text):
            m = self.pattern_detector.get_matched_patterns(text)
            ps = self.pattern_detector.get_score(text)
            return self._result(1.0, ps, stat_total, True, m)

        # Layers 2-3: Semantic + normalized
        sem = self._check_semantic_layers(text)
        if sem:
