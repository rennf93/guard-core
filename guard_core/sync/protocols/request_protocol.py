...
@runtime_checkable
class SyncBoundedBodyReader(Protocol):
    ...
    HOW: the blocking mirror of ``BoundedBodyReader``. Adapters implement
    this alongside ``SyncGuardRequest`` by reading from their underlying
    stream and stopping at ``max_bytes``. It is a separate, optional
    protocol: an adapter that only implements ``SyncGuardRequest`` is still
    fully valid, and callers must treat the absence of this capability as
    "cannot bound the read" and fall back accordingly rather than reading
    the body in full.
