...
def invoke_block_hook(
    hook: Callable[[SyncGuardRequest, dict[str, Any]], Any] | None,
    ...
    if inspect.isawaitable(result):
        raise TypeError(
            "async on_block hook not supported in sync (WSGI) deployments; "
            "supply a sync on_block hook"
        )
