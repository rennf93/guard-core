...
### Route-Config Revision Rebuild

`BaseSecurityDecorator` holds a `RouteConfigRevision` counter -- its own mutable cell, independent of `SecurityConfig._revision` -- that starts at zero for every decorator instance. `RouteConfig.__setattr__` bumps that counter on every attribute assignment except during `__init__` itself, gated behind a private `_initialized` flag that only flips `True` after the constructor's own ~25 field assignments finish, so decorating a fresh route touches the counter zero times for its own defaults, the same way `SecurityConfig`'s Pydantic-populated `__init__` never touches its overridden `__setattr__`. `_ensure_route_config` is the single insertion point for `_route_configs`; the counter is also bumped there explicitly, the moment a route id is first seen, so a route decorated after the pipeline was already built is observable on its own, independent of whatever attribute assignment the calling decorator makes next. `build_default_pipeline` reads `middleware.guard_decorator.route_config_revision` through a `route_config_revision` callable it hands the pipeline alongside `config` and `watched_container_fields`; `_is_stale()` compares that against the value recorded at the last build as one more integer comparison after the existing two, so the hot path stays O(1) in the number of watched fields and never touches `_route_configs` itself -- comparing container sizes across every route config on every request would make the steady-state cost proportional to route count, which the counter design avoids entirely. When `middleware.guard_decorator` is `None` the callable returns `None` on both sides of the comparison, so a middleware with no decorator handle behaves exactly as before this mechanism existed.

In-place mutation of a container *inside* a `RouteConfig` -- `route_config.custom_validators.append(...)`, `route_config.required_headers["X-Api-Key"] = "required"` -- does not go through `RouteConfig.__setattr__` any more than `config.blocked_user_agents.append(...)` goes through `SecurityConfig.__setattr__`. All six route-driven predicates (`authentication`, `custom_validators`, `referrer`, `request_size_content`, `required_headers`, `time_window`) read their `RouteConfig` fields as `bool(...)` or `is not None`, the same truthiness-only pattern the `SecurityConfig` side relies on, so the fix takes the same shape but a different form: recomputing a length signature across every route config on every request would scale with route count, which the hot-path constraint rules out, so instead `RouteConfig.__setattr__` wraps values assigned to exactly the mutable-container fields these six predicates read -- `custom_validators`, `require_referrer`, `allowed_content_types` (lists) and `required_headers`, `time_restrictions` (dicts) -- in a `list`/`dict` subclass whose mutating methods (`append`, `extend`, `insert`, `remove`, `pop`, `clear`, `__setitem__`, `__delitem__` for lists; the dict equivalents plus `update`, `setdefault`, `popitem`) bump the same shared counter directly, in the method call itself, rather than being detected by scanning afterward. Wrapping happens unconditionally, including for the empty `[]`/`{}` the constructor assigns, so the very first container object a decorator gets back from `_ensure_route_config` is already tracked -- deferring wrapping until after `_initialized` would leave that first container, the one most decorators mutate in place, permanently unobserved. `max_request_size` and `auth_required` are read by the same six predicates but are `int | None` and `str | None`; both are immutable, so whole-value assignment -- already covered -- is the only way either changes.

**Residual**: closing the six route-driven predicates does not close every `RouteConfig` container. `cloud_provider`/`cloud_ip_refresh` read `RouteConfig.block_cloud_providers` (a `set`), `rate_limit` reads `RouteConfig.geo_rate_limits` (a `dict`), and `user_agent` reads `RouteConfig.blocked_user_agents` (a `list`) -- all three by truthiness, same as the six -- but none of the three is purely route-driven: each `applies_to` is already an `or` against a `SecurityConfig`-level condition (`cloud_blocking_enabled(config)`; `config.enable_rate_limiting or bool(config.endpoint_rate_limits)`; `bool(config.blocked_user_agents)`) or `enable_dynamic_rules`, so none was in this unit's scope. Whole-value re-assignment of any of those three fields is covered, since the route-config revision bumps on every `RouteConfig.__setattr__` regardless of field name; in-place mutation (`.add()`, `route_config.geo_rate_limits["/x"] = (1, 60)`, `.append()`) of those three specific fields is not, since they are not wrapped. `enable_dynamic_rules=True` remains the supported escape hatch for the checks it covers regardless.
