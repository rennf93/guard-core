...
Framework adapters (fastapi-guard, flaskapi-guard, djapi-guard) build the event bus and metrics collector through the factory methods on `HandlerInitializer` so the composite handler and event filter are wired automatically:
