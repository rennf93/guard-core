...
- **Body-read timeout (async tree only).** Nothing bounded the wait on an adapter's `read_body_prefix`/`body` call: a stalled SSE producer, a long-poll that never yields, or a buggy adapter implementation left `_safe_read` awaiting forever, pinning the request (and, at volume, every worker handling one) indefinitely -- a real, unbounded DoS with no recovery short of a process restart. `_safe_read` (`guard_core/utils.py`) now wraps the call in `asyncio.wait_for(reader(), timeout=timeout)`; on timeout it degrades to the identical fail-closed, could-not-evaluate outcome already used when the reader raises, through whichever caller's existing throttled logging already covers that path (`BehaviorTracker._log_body_unavailable`'s `TTLCache` on the response side; the request side already returns `None` silently for a raising reader and continues to). This bound is configurable via `SecurityConfig.body_read_timeout` (default 3.0 seconds) and applies uniformly to `BoundedBodyReader.read_body_prefix`, `BoundedResponseBodyReader.read_body_prefix`, and the plain `GuardRequest.body` read, since all three route through `_safe_read` -- **in the ASYNC guard_core tree only.** **The SYNC tree (`guard_core.sync`) does not bound this wait at all, by design.** `_safe_read` in `guard_core/sync/utils.py` calls the adapter's read directly (`try: return reader() except Exception: return None`): `SyncGuardRequest.body`, `SyncBoundedBodyReader.read_body_prefix`, and `SyncBoundedResponseBodyReader.read_body_prefix` block the calling thread for exactly as long as the adapter takes, the same as any other slow call in a WSGI application, and `SecurityConfig.body_read_timeout` has no effect on the sync tree at all. An earlier iteration of this fix instead bounded the sync *caller's* wait with a `threading.Semaphore(16)` (`_body_read_slots`) that handed each read to a fresh `daemon=True` thread and called `future.result(timeout=timeout)`; that machinery -- the semaphore, the worker-count constant, the daemon-thread spawn, and the matching `scripts/unasync.py` code-generation substitution -- is removed entirely, because it traded the hang for two worse failure modes: a slot was released only when the background thread's blocking call actually finished, not when the caller's wait timed out, so a sustained-stall attacker permanently consumed one of the 16 slots per stalled connection with no recovery short of a worker restart; and because `acquire(blocking=False)` failed closed the instant all 16 slots were occupied, with no queueing and no logging on that path, as few as 17 ordinary concurrent body reads -- no attacker involved, just normal traffic -- silently disabled body-based detection for the 17th request onward, for a fixed pool size no `SecurityConfig` field could raise. A deployment that needs a bounded wait on the sync tree must configure its WSGI server's own request timeout (gunicorn `--timeout`, uWSGI `harakiri`); guard-core does not, and cannot safely, provide one itself for a blocking adapter call without reintroducing the same class of failure.
...
- `model_copy(update={...})` on `SecurityConfig` bypasses both the shadow-blocklist validator and `__setattr__`, so it can silently produce a shadowed `blocked_countries`/`whitelist_countries` configuration with no warning. Unlike the `exclude_paths` gap above, this one is accepted rather than closed: a shadowed country configuration degrades to the already-documented, intentional whitelist-wins behaviour rather than disabling the security pipeline, so it does not carry the same blast radius. guard-core itself never calls `model_copy` on `SecurityConfig`; a caller that builds configs via `model_copy(update=...)` should re-validate the result, for example `SecurityConfig.model_validate(cfg.model_dump())`.
