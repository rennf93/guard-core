...
- The deferral flagged above is made, with the hole it would have had closed. `_mute_pydantic_plugin_instrumentation()` moved out of `guard_core/__init__.py` into its own module, `guard_core/_pydantic_plugin_mute.py` (added to `scripts/unasync.py`'s `SKIP_SRC`, so it is shared between the async and sync trees exactly like `models.py`), gained a module-level applied flag so repeated calls are free after the first, and is no longer called at import time. `SecurityConfig.to_agent_config()` calls it first thing, covering the `enable_agent=True` path. Auditing every guard-agent telemetry-model construction site (`middleware_events.py`, `metrics.py`, `decorators/base.py`, `suspatterns_handler.py`) found that all of them gate on a handler wired in exclusively through `HandlerInitializer`'s `.initialize_agent(...)` calls, and that `HandlerInitializer.initialize_agent_integrations()` builds a real, working `CompositeAgentHandler` and wires it into every one of those consumers whenever `enable_otel` or `enable_logfire` is set, with no agent handler at all — `enable_enrichment` requires `enable_agent=True` by validator, but OTel/Logfire alone do not, so that path could construct a validated `SecurityEvent` without ever calling `to_agent_config()`. `initialize_agent_integrations()` now calls the same idempotent helper right after its existing early-return guard, closing that gap; both call sites share one implementation, not a copy. Measured on this branch (Python 3.10.19, warm filesystem, `guard-agent`/`logfire`/`cryptography` installed): `import guard_core` dropped from roughly 262-287ms to roughly 1.6-2ms, and `guard_agent`/`cryptography` are absent from `sys.modules` after a bare import (`tests/test_import_cost.py` now asserts this alongside the existing `aiohttp`/`maxminddb`/`redis` check). `guard_core._mute_pydantic_plugin_instrumentation` keeps resolving through the existing PEP 562 `__getattr__` for back-compat. The mute now covers only the telemetry models guard-core itself constructs, at the point it starts constructing them, not every guard-agent model unconditionally; a host application that builds `SecurityEvent`/`SecurityMetric`/`EventBatch` through a path that never goes through a guard-core `SecurityConfig` is not covered, and the shipped skill docs (`SKILL.md`, `references/telemetry.md`) now say so.
