...
- Mutating `SecurityConfig` after the middleware has already built its pipeline no longer resurrects a check that build time eliminated. `build_default_pipeline` filters `DEFAULT_CHECK_CLASSES` through `applies_to` once, when the pipeline is built, not on every request: a default `SecurityConfig()` with no route decorators builds `["route_config", "ip_security", "rate_limit", "suspicious_activity"]`, and setting `config.blocked_user_agents = ["badbot"]` on that same config afterward leaves the existing pipeline at those same four checks, while a freshly built pipeline from the mutated config includes `user_agent`. Before 3.10.0 every check was always present and read the live config on every request, so the same mutation took effect immediately with no rebuild needed. `SecurityConfig` is a plain mutable Pydantic model with no `frozen` setting, so this is reachable from any code that holds a reference to a config already handed to a middleware. The supported way to change enforcement at runtime is unaffected: `enable_dynamic_rules=True` keeps all seven dynamically-mutable checks (`emergency_mode`, `cloud_ip_refresh`, `ip_security`, `cloud_provider`, `user_agent`, `rate_limit`, `suspicious_activity`) regardless of what the rest of the config says. A deployment that mutates `SecurityConfig` fields directly outside that mechanism, after startup, should either turn on `enable_dynamic_rules` or rebuild the middleware (and therefore its pipeline) after the change.
- `import guard_core.handlers` followed by attribute access on a submodule name, for example `guard_core.handlers.ipban_handler` itself rather than the `IPBanManager` class it defines, now raises `AttributeError: module 'guard_core.handlers' has no attribute 'ipban_handler'` unless something else already imported that submodule first. It worked before because the package eagerly imported every submodule at `import guard_core.handlers` time. `from guard_core.handlers.ipban_handler import ip_ban_manager` is unaffected and keeps working exactly as before, and it is the form every consumer in the ecosystem already uses, so this only affects code doing attribute access directly on the `guard_core.handlers` package.
...
- The pydantic-plugin instrumentation mute now runs the first time telemetry actually initialises, through `SecurityConfig.to_agent_config()` or `HandlerInitializer.initialize_agent_integrations()`, instead of at `import guard_core`. This is the change that takes cold `import guard_core` from roughly 262ms to roughly 1.6ms when `guard-agent`/`logfire` are installed. A host application that constructs guard-agent's `SecurityEvent`, `SecurityMetric`, or `EventBatch` models through a path that never goes through a guard-core `SecurityConfig` is no longer covered by the mute and must call it itself; every construction site inside guard-core already goes through `get_telemetry_model(name)`, which mutes and returns the class in one call.
