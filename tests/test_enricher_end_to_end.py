...
@pytest.mark.asyncio
async def test_end_to_end_enrichment_on_event_through_composite() -> None:
...
@pytest.mark.asyncio
async def test_end_to_end_enrichment_on_metric_through_composite() -> None:
    config = SecurityConfig(
        enable_agent=True,
        agent_api_key="a" * 10,
...
@pytest.mark.asyncio
async def test_muted_event_is_not_enriched_or_dispatched() -> None:
    config = SecurityConfig(
        enable_agent=True,
        agent_api_key="a" * 10,
        enable_enrichment=True,
