...
async def test_check_blocked_countries_country_not_blocked() -> None:
    config = MagicMock()
    ...
    with patch(
        ...
        result = await _check_blocked_countries("1.2.3.4", config, geo_ip)
    assert result is True


async def test_check_blocked_countries_country_blocked() -> None:
    config = MagicMock()
    config.blocked_countries = ["CN"]
    geo_ip = MagicMock()
    with patch(
        ...
        async def _async_true(*_a: object, **_kw: object) -> bool:
            return True
...
async def test_check_blocked_countries_no_rules_skips_lookup() -> None:
    config = MagicMock()
...
async def test_detect_penetration_attempt_excluded_header_still_detects_xss() -> None:
    request = MagicMock()
...
async def test_detect_penetration_attempt_non_excluded_header_hit() -> None:
    request = MagicMock()
    request.client_host = "1.2.3.4"
    request.query_params = {}
    request.url_path = "/"
...
async def test_detect_penetration_attempt_scan_body_disabled_skips_the_body() -> None:
    request = MagicMock()
    request.client_host = "1.2.3.4"
    request.query_params = {}
    request.url_path = "/"
