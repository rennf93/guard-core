...
def _header_request(payload: str) -> MockGuardRequest:
    return MockGuardRequest(headers={"x-note": payload})
...
_SURFACE_BUILDERS: dict[str, Callable[[str], MockGuardRequest]] = {
...
async def test_nested_array_nosql_operator_detected_on_every_surface(
    surface: str,
) -> None:
...
async def test_form_value_json_password_redacted_in_detection_log(
