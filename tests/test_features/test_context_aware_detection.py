...
@pytest.mark.asyncio
async def test_xml_injection_fires_on_query_param() -> None:
    manager = SusPatternsManager()
    result = await manager.detect(
...
@pytest.mark.asyncio
async def test_xss_fires_on_url_path() -> None:
    manager = SusPatternsManager()
    result = await manager.detect(
        "<script>alert(1)</script>",
        "127.0.0.1",
...
@pytest.mark.asyncio
async def test_cmd_injection_fires_on_header() -> None:
    manager = SusPatternsManager()
    result = await manager.detect(
        "; whoami",
        "127.0.0.1",
...
@pytest.mark.asyncio
async def test_ssrf_fires_on_url_path() -> None:
    manager = SusPatternsManager()
    result = await manager.detect(
...
@pytest.mark.asyncio
async def test_ssrf_fires_on_header() -> None:
    manager = SusPatternsManager()
    result = await manager.detect(
        "http://169.254.169.254/latest/meta-data/",
        "127.0.0.1",
...
@pytest.mark.asyncio
async def test_template_fires_on_url_path() -> None:
    manager = SusPatternsManager()
    result = await manager.detect(
        "{{7*7}}",
        "127.0.0.1",
...
@pytest.mark.asyncio
async def test_deserialization_fires_on_header() -> None:
    manager = SusPatternsManager()
    result = await manager.detect(
        'O:8:"stdClass":0:{}',
        "127.0.0.1",
