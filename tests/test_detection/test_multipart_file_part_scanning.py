...
async def test_malicious_upload_filename_detected(filename: str) -> None:
...
async def test_malicious_upload_content_detected(content: str, category: str) -> None:
...
async def test_base64_transfer_encoded_upload_content_detected(
...
async def test_base64_transfer_encoded_upload_survives_boundary_mismatch(
    content: str, category: str
) -> None:
...
async def test_wellformed_upload_with_decoy_prefixed_base64_content_detected(
    content: str, category: str
) -> None:
...
async def test_wellformed_upload_with_fragmented_below_floor_base64_content_detected(
    content: str, category: str
) -> None:
...
async def test_benign_upload_filename_and_content_not_detected() -> None:
    request = _body_request(_file_part_body("note.txt", "hello world"), _CONTENT_TYPE)
    result = await detect_penetration_attempt(request, _CONFIG)
    assert result.is_threat is False


async def test_excluded_field_name_skips_malicious_file_part() -> None:
...
async def test_no_name_file_part_still_detected_when_excluded_by_fallback_label() -> (
    None
):
...
async def test_nested_multipart_no_real_name_not_suppressed_by_exclusion() -> None:
...
async def test_no_content_disposition_part_malicious_content_detected() -> None:
...
async def test_no_content_disposition_part_excluded_field_does_not_suppress() -> None:
    raw_body = _no_disposition_part_body("<script>alert(1)</script>").decode()
    detected, trigger, threats = await _scan_multipart_body(
        raw_body, _CONTENT_TYPE, {"file"}, None, "127.0.0.1", "corr-1", "WARNING"
    )
...
async def test_text_field_without_filename_still_detected_as_before() -> None:
    request = _body_request(
...
async def test_text_field_without_filename_benign_not_detected() -> None:
...
async def test_nested_multipart_mixed_matches_flat_equivalent() -> None:
    ...
    nested_result = await detect_penetration_attempt(nested_request, _CONFIG)
    flat_result = await detect_penetration_attempt(flat_request, _CONFIG)
...
async def test_filename_with_escaped_embedded_double_quote_still_detected() -> None:
...
async def test_filename_with_embedded_single_quote_still_detected() -> None:
    body = _raw_disposition_file_part_body(
...
async def test_filename_with_embedded_newline_via_rfc2231_still_detected() -> None:
    body = _raw_disposition_file_part_body(
...
async def test_binary_file_content_not_detected() -> None:
...
async def test_malicious_filename_still_detected_with_binary_content() -> None:
    binary_blob = (bytes(range(256)) * 100).decode("latin-1")
    request = _body_request(
...
async def test_benign_binary_corpus_produces_no_threats(
...
async def test_padded_webshell_detected_despite_binary_padding() -> None:
...
async def test_padded_webshell_detected_with_majority_binary_padding() -> None:
    payload = b"<?php system($_GET['cmd']); ?>"
...
def test_empty_payload_file_part_yields_no_payload_entries() -> None:
    from guard_core._utils.body_form_scan import _part_payload_entries

    assert _part_payload_entries("file", "file", "empty.bin", "") == []
    assert _part_payload_entries(None, "file", None, "") == []


async def test_empty_file_part_content_not_detected() -> None:
