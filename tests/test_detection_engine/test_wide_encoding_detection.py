...
async def _detect(body: bytes, content_type: str = "text/plain") -> DetectionResult:
    request = MockGuardRequest(
        method="POST",
        headers={"content-length": str(len(body)), "content-type": content_type},
        body_content=body,
    )
    return await detect_penetration_attempt(
...
async def test_wide_encoding_family_payload_is_detected(
    family: str, encoding: str
) -> None:
    payload = _FAMILY_PAYLOADS[family]
    body = payload.encode(encoding)

    result = await _detect(body)

    assert result.is_threat is True
    assert family in result.threat_categories


async def test_utf16_with_bom_xss_payload_is_detected() -> None:
...
async def test_utf16_le_with_embedded_cjk_character_xss_payload_is_detected() -> None:
...
async def test_wide_encoding_pipeline_detection_traces_to_null_removal() -> None:
    payload = _FAMILY_PAYLOADS["sqli"]
    wide_bytes = payload.encode("utf-32-be")

    result = await _detect(wide_bytes)
