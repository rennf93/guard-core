...
@pytest.mark.asyncio
async def test_identity_populates_project_and_service_from_config() -> None:
    cfg = SecurityConfig(
        enable_agent=True,
        agent_api_key="a" * 10,
    ...
    await _enricher(cfg).enrich_event(event)
...
@pytest.mark.asyncio
async def test_identity_adds_deployment_environment_when_set() -> None:
    cfg = SecurityConfig(
        enable_agent=True,
        agent_api_key="a" * 10,
...
@pytest.mark.asyncio
async def test_identity_omits_project_id_when_unset() -> None:
...
@pytest.mark.asyncio
async def test_identity_applied_to_metric_tags() -> None:
    cfg = SecurityConfig(
        enable_agent=True,
        agent_api_key="a" * 10,
    ...
    await _enricher(cfg).enrich_metric(metric)
...
@pytest.mark.asyncio
async def test_identity_omits_deployment_env_when_key_missing() -> None:
    cfg = SecurityConfig(
