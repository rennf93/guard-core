...
@pytest.mark.asyncio
async def test_rule_update_loop_exits_when_already_stopped(
    config: SecurityConfig,
) -> None:
    DynamicRuleManager._instance = None
    ...
    await manager._rule_update_loop()
...
@pytest.mark.asyncio
async def test_apply_pattern_rules_logs_rejected_patterns(
    config: SecurityConfig, caplog: pytest.LogCaptureFixture
) -> None:
    DynamicRuleManager._instance = None
    ...
    with patch(
        ...
        mock_patterns.add_pattern = AsyncMock(side_effect=[True, False])
...
@pytest.mark.asyncio
async def test_apply_pattern_rules_all_rejected_logs_no_added_line(
    config: SecurityConfig, caplog: pytest.LogCaptureFixture
) -> None:
    DynamicRuleManager._instance = None
    ...
    with patch(
        ...
        mock_patterns.add_pattern = AsyncMock(return_value=False)
...
@pytest.mark.asyncio
async def test_apply_feature_toggles_auto_ban_overrides(
    config: SecurityConfig, caplog: pytest.LogCaptureFixture
) -> None:
    DynamicRuleManager._instance = None
...
@pytest.mark.asyncio
async def test_apply_feature_toggles_auto_ban_none_values(
    config: SecurityConfig, caplog: pytest.LogCaptureFixture
) -> None:
    DynamicRuleManager._instance = None
...
@pytest.mark.asyncio
async def test_apply_feature_toggles_rate_limit_auto_ban_false_override(
    config: SecurityConfig, caplog: pytest.LogCaptureFixture
) -> None:
    DynamicRuleManager._instance = None
...
@pytest.mark.asyncio
async def test_apply_rules_emergency_mode_halves_overridden_auto_ban_threshold(
    config: SecurityConfig,
    mock_agent_handler: AsyncMock,
    sample_rules: DynamicRules,
) -> None:
