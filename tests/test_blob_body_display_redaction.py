...
def _blob_request(body: bytes, content_type: str | None) -> MockGuardRequest:
...
async def test_text_plain_pair_body_redacts_sensitive_pair_keeps_benign_pair(
...
async def test_missing_content_type_pair_body_still_redacted(
    caplog: pytest.LogCaptureFixture,
) -> None:
    config = SecurityConfig()
    body = f"password={_SECRET}&note=<script>alert(1)</script>".encode()
...
async def test_xml_sensitive_element_redacted_non_sensitive_element_kept(
    caplog: pytest.LogCaptureFixture,
) -> None:
    config = SecurityConfig()
...
async def test_text_body_valid_json_redacted_like_json_content_type(
    caplog: pytest.LogCaptureFixture,
) -> None:
    config = SecurityConfig()
...
async def test_custom_sensitive_body_field_only_pair_redacted(
    caplog: pytest.LogCaptureFixture,
) -> None:
...
async def test_blob_body_all_pair_separators_redacted(
    caplog: pytest.LogCaptureFixture,
) -> None:
    config = SecurityConfig()
...
async def test_non_sensitive_blob_body_shown_verbatim(
    caplog: pytest.LogCaptureFixture,
) -> None:
    config = SecurityConfig()
