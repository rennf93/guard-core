...
async def test_stop_shuts_down_only_owned_providers(config: MagicMock) -> None:
    with patch("guard_core.core.events.otel_handler._otel_available", True):
        handler = OtelHandler(config)
...
async def test_stop_does_not_shut_down_borrowed_providers(config: MagicMock) -> None:
    with patch("guard_core.core.events.otel_handler._otel_available", True):
        handler = OtelHandler(config)
        handler._tracer = MagicMock()
        handler._meter = MagicMock()
...
async def test_start_matrix_no_ambient_tracer_no_ambient_meter_owns_both(
    config: MagicMock,
...
async def test_start_matrix_ambient_tracer_no_ambient_meter_borrows_tracer_owns_meter(
    config: MagicMock,
    caplog: pytest.LogCaptureFixture,
) -> None:
    import logging
...
async def test_start_matrix_no_ambient_tracer_ambient_meter_owns_tracer_borrows_meter(
    config: MagicMock,
    caplog: pytest.LogCaptureFixture,
) -> None:
    import logging
...
async def test_start_matrix_ambient_tracer_and_ambient_meter_borrows_both(
    config: MagicMock,
    caplog: pytest.LogCaptureFixture,
) -> None:
    import logging
