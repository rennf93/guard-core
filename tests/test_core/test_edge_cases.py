...
async def test_get_redis_request_count_no_redis_handler() -> None:
    config = SecurityConfig()
...
async def test_get_validated_cors_config_no_cors_config() -> None:
...
async def test_remove_default_pattern_not_found() -> None:
...
async def test_remove_default_pattern_invalid_index() -> None:
    handler = SusPatternsManager()
...
async def _raising_recursion_error_detect(
    *_args: Any, **_kwargs: Any
) -> dict[str, Any]:
    raise RecursionError("maximum recursion depth exceeded")


async def test_check_value_enhanced_recursion_error_escapes_the_fallback() -> None:
    ...
    with patch.object(
        ...
        with pytest.raises(RecursionError):
            await _check_value_enhanced(
                "' OR '1'='1", "request_body", "127.0.0.1", "corr-4"
            )
...
async def test_check_value_enhanced_empty_threats_list() -> None:
    ...
    with patch(
        ...
        mock_handler.detect = AsyncMock(return_value={"is_threat": True, "threats": []})
