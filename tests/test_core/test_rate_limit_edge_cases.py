...
async def test_check_geo_rate_limit_exceeded_without_decorator_uses_event_bus(
    rate_limit_check: RateLimitCheck,
    ...
    geo_handler.get_country.return_value = "US"
    security_config.geo_ip_handler = geo_handler
    mock_middleware.rate_limit_handler.check_rate_limit = AsyncMock(
        return_value=Mock(status_code=429)
    )
    ...
    mock_middleware.event_bus.send_middleware_event.assert_awaited_once()
...
async def test_check_geo_rate_limit_exceeded_with_decorator_uses_send_decorator_event(
    rate_limit_check: RateLimitCheck,
    mock_middleware: Mock,
    security_config: SecurityConfig,
) -> None:
    ...
    result = await rate_limit_check._check_geo_rate_limit(
        request, "1.2.3.4", route_config
    )

    assert result is not None
    mock_middleware.event_bus.send_middleware_event.assert_not_called()
    recording_agent.send_event.assert_awaited_once()
...
async def test_endpoint_rate_limit_redacts_sensitive_endpoint_path(
    rate_limit_check: RateLimitCheck,
    mock_request: Mock,
    security_config: SecurityConfig,
) -> None:
