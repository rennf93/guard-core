...
async def test_knob_off_default_never_bans_on_violation() -> None:
    config = SecurityConfig(
    ...
    await check.check(_make_request("1.1.1.1"))
    await check.check(_make_request("1.1.1.1"))
...
async def test_per_category_threat_ban_config_override_honored() -> None:
    ...
    await check.check(_make_request("3.3.3.3"))
...
async def test_passive_mode_does_not_feed_counter_or_ban() -> None:
    config = SecurityConfig(
        passive_mode=True,
        enable_rate_limiting=True,
        rate_limit=1,
    ...
    result = await check.check(_make_request("4.4.4.4"))
...
async def test_loopback_ip_not_banned_despite_threshold_crossing() -> None:
    ...
    try:
        ...
        await check.check(_make_request("127.0.0.1"))
...
async def test_enable_ip_banning_false_suppresses_ban() -> None:
    ...
    await check.check(_make_request("5.5.5.5"))
