...
async def _blocking_custom_validator(request: object) -> MockGuardResponse:
...
async def _assert_eliminated_check_cannot_block(
    check_class: type[SecurityCheck],
    config: SecurityConfig,
    route_configs: tuple[RouteConfig, ...],
    route_id: str | None,
    adversarial_requests: Sequence[MockGuardRequest],
) -> None:
    ...
    for request in adversarial_requests:
        ...
        await route_config_check.check(request)
        result = await check_under_test.check(request)
...
async def test_emergency_mode_cannot_block_once_eliminated() -> None:
    for emergency_mode, enable_dynamic_rules in product(_BOOL_AXIS, _BOOL_AXIS):
        ...
        if EmergencyModeCheck.applies_to(config, ()) is False:
            await _assert_eliminated_check_cannot_block(
                EmergencyModeCheck,
                config,
                (),
                None,
                [MockGuardRequest(client_host="203.0.113.10")],
            )


async def test_https_enforcement_cannot_block_once_eliminated() -> None:
    for enforce_https, route_require_https in product(_BOOL_AXIS, _BOOL_AXIS):
        ...
        if HttpsEnforcementCheck.applies_to(config, route_configs) is False:
            await _assert_eliminated_check_cannot_block(
                ...
                route_configs,
                "route-0",
                [MockGuardRequest(client_host="203.0.113.11", scheme="http")],
            )


async def test_request_logging_cannot_block_once_eliminated() -> None:
    for log_request_level in (None, "INFO"):
        ...
        if RequestLoggingCheck.applies_to(config, ()) is False:
            await _assert_eliminated_check_cannot_block(
                ...
                (),
                None,
                [MockGuardRequest(client_host="203.0.113.12")],
            )


async def test_custom_request_cannot_block_once_eliminated() -> None:
    for custom_request_check in (None, _blocking_custom_request_check):
        ...
        if CustomRequestCheck.applies_to(config, ()) is False:
            await _assert_eliminated_check_cannot_block(
                ...
                (),
                None,
                [MockGuardRequest(client_host="203.0.113.13")],
            )


async def test_request_size_content_cannot_block_once_eliminated() -> None:
...
async def test_required_headers_cannot_block_once_eliminated() -> None:
    for required_headers in ({}, {"X-Api-Key": "required"}):
        ...
        if RequiredHeadersCheck.applies_to(config, route_configs) is False:
            await _assert_eliminated_check_cannot_block(
                ...
                route_configs,
                "route-0",
                [MockGuardRequest(client_host="203.0.113.15", headers={})],
            )
    ...
    for auth_required in (None, "bearer"):
        ...
        if AuthenticationCheck.applies_to(config, route_configs) is False:
            await _assert_eliminated_check_cannot_block(
                ...
                route_configs,
                "route-0",
                [MockGuardRequest(client_host="203.0.113.16", headers={})],
            )


async def test_referrer_cannot_block_once_eliminated() -> None:
        ...
        if ReferrerCheck.applies_to(config, route_configs) is False:
            await _assert_eliminated_check_cannot_block(
                ...
                [
                    ...
                    MockGuardRequest(client_host="203.0.113.17", headers={}),
                ],
            )


async def test_custom_validators_cannot_block_once_eliminated() -> None:
    for custom_validators in ([], [_blocking_custom_validator]):
        ...
        if CustomValidatorsCheck.applies_to(config, route_configs) is False:
            await _assert_eliminated_check_cannot_block(
                ...
                route_configs,
                "route-0",
                [MockGuardRequest(client_host="203.0.113.18")],
            )


async def test_time_window_cannot_block_once_eliminated() -> None:
    for time_restrictions in (
        ...
        if TimeWindowCheck.applies_to(config, route_configs) is False:
            await _assert_eliminated_check_cannot_block(
                ...
                route_configs,
                "route-0",
                [MockGuardRequest(client_host="203.0.113.19")],
            )
...
async def test_cloud_ip_refresh_cannot_block_once_eliminated() -> None:
    ...
    for config, route_configs in _cloud_axis_combinations():
        if CloudIpRefreshCheck.applies_to(config, route_configs) is False:
            await _assert_eliminated_check_cannot_block(
                ...
                route_configs,
                "route-0",
                [MockGuardRequest(client_host="3.0.0.9")],
            )


async def test_cloud_provider_cannot_block_once_eliminated() -> None:
    cloud_handler.ip_ranges["AWS"].add(ip_network("3.0.0.0/8"))
    for config, route_configs in _cloud_axis_combinations():
...
async def test_user_agent_cannot_block_once_eliminated() -> None:
...
async def test_rate_limit_cannot_block_once_eliminated() -> None:
    for (
        ...
        if RateLimitCheck.applies_to(config, route_configs) is False:
            await _assert_eliminated_check_cannot_block(
                ...
                route_configs,
                "route-0",
                [MockGuardRequest(client_host="203.0.113.21")],
            )


async def test_suspicious_activity_cannot_block_once_eliminated() -> None:
    for (
