...
async def _blocking_custom_request_check(request: object) -> GuardResponse:
    return MockGuardResponse("blocked-by-custom-request-check", 451)


async def _blocking_custom_validator(request: object) -> GuardResponse:
...
async def _run_pipeline(
    checks: list[SecurityCheck], request: MockGuardRequest
...
async def _run_side(
    ...
    request = MockGuardRequest(**case.request_kwargs)
    if case.request_route_id is not None:
        request.state.guard_route_id = case.request_route_id

    return await _run_pipeline(checks, request)


@pytest.mark.parametrize("case", CORPUS, ids=[case.id for case in CORPUS])
async def test_eliminated_pipeline_blocks_everything_the_full_pipeline_blocks(
    ...
    with (
        patch.object(cloud_handler, "schedule_refresh", AsyncMock(return_value=False)),
        context,
    ):
        reference_result = await _run_side(case, eliminated=False)
        eliminated_result = await _run_side(case, eliminated=True)
