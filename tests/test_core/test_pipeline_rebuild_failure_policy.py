...
async def test_execute_blocks_when_fail_secure_and_rebuild_raises() -> None:
    ...
    result = await pipeline.execute(_mock_request())
...
async def test_execute_continues_on_last_known_good_checks_when_not_fail_secure() -> (
    None
):
    ...
    middleware.create_error_response.assert_not_awaited()
    assert pipeline.checks == [check]


async def test_transient_rebuild_failure_recovers_on_a_later_request() -> None:
    config = SecurityConfig(fail_secure=False)
    middleware = _stub_middleware()
    ...
    await pipeline.execute(_mock_request())
...
async def test_execute_reraises_when_fail_secure_and_no_known_good_checks_exist() -> (
    None
):
    config = SecurityConfig(fail_secure=True)
