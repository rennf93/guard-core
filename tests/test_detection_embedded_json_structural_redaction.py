...
def _surface_request(surface: str, value: str) -> MockGuardRequest:
    if surface == "header":
        return MockGuardRequest(headers={"X-Custom": value})
    if surface == "query_param":
        return MockGuardRequest(query_params={"data": value})
    return MockGuardRequest(path=value)
...
async def test_attack_in_non_sensitive_field_keeps_sibling_secret_redacted(
...
@pytest.mark.parametrize("surface", _SURFACES)
async def test_nested_two_levels_sensitive_key_redacted(
    surface: str, caplog: pytest.LogCaptureFixture, mock_agent: MagicMock
) -> None:
...
@pytest.mark.parametrize("surface", _SURFACES)
async def test_object_under_sensitive_key_nested_nonsensitive_leaf_redacted(
    surface: str, caplog: pytest.LogCaptureFixture, mock_agent: MagicMock
) -> None:
...
@pytest.mark.parametrize("surface", _SURFACES)
async def test_array_of_objects_under_sensitive_key_nested_leaf_redacted(
    surface: str, caplog: pytest.LogCaptureFixture, mock_agent: MagicMock
) -> None:
...
@pytest.mark.parametrize("surface", _SURFACES)
async def test_array_of_objects_sensitive_key_redacted(
    surface: str, caplog: pytest.LogCaptureFixture, mock_agent: MagicMock
) -> None:
...
@pytest.mark.parametrize("surface", _SURFACES)
async def test_sensitive_value_longer_than_cap_never_partially_leaks(
    surface: str, caplog: pytest.LogCaptureFixture, mock_agent: MagicMock
) -> None:
    padding = "X" * 150
...
@pytest.mark.parametrize("surface", _SURFACES)
async def test_json_key_trips_pattern_key_stays_raw_for_non_sensitive_parent(
...
@pytest.mark.parametrize("surface", _SURFACES)
async def test_attack_only_in_sensitive_field_outer_line_redacted(
    surface: str, caplog: pytest.LogCaptureFixture, mock_agent: MagicMock
) -> None:
...
async def test_sensitive_parent_blanket_redacts_nested_json(
    surface: str, caplog: pytest.LogCaptureFixture, mock_agent: MagicMock
) -> None:
