...
async def test_precomputed_decoded_matches_recomputed_decode(content: str) -> None:
    state = sus_patterns_handler._detection_state
    (
        processed,
        decode_budget_exhausted,
        decoded,
    ) = await sus_patterns_handler._preprocess_content(content, "corr-1", state=state)

    via_precomputed = await sus_patterns_handler._check_url_decoded_view_patterns(
        content,
        "127.0.0.1",
        "request_body",
        "corr-1",
    ...
    via_recomputed = await sus_patterns_handler._check_url_decoded_view_patterns(
...
async def test_check_url_decoded_view_patterns_without_precomputed_still_detects() -> (
    None
):
    state = sus_patterns_handler._detection_state
    ...
    (
        ...
        timeouts,
        budget_exhausted,
    ) = await sus_patterns_handler._check_url_decoded_view_patterns(
        "x%0A/bin/sh%20-c%20id",
        "127.0.0.1",
        "request_body",
        "corr-1",
