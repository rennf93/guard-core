...
async def test_decoded_view_check_is_inert_in_legacy_mode() -> None:
    original_instance = SusPatternsManager._instance
    original_config = SusPatternsManager._config
    ...
    try:
        ...
        result = await legacy_manager.detect(
            "%2e%2e%2f", "203.0.113.9", context="url_path"
        )
...
async def test_decoded_view_check_skipped_outside_path_traversal_contexts(
    manager: SusPatternsManager,
) -> None:
    threats = await _new_pattern_threats(manager, "%2e%2e%2f", context="header")
    assert threats == []
