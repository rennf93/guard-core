...
async def test_ldap_null_byte_attr_name_start_benign_input_not_detected(
    manager: SusPatternsManager,
) -> None:
    result = await _manager_detect("1=x", manager)
    assert not _null_byte_threats(result)
...
async def test_ldap_null_byte_finditer_lead_fail_input_not_detected(
    manager: SusPatternsManager,
) -> None:
    result = await _manager_detect("! =a*)" + _NUL_RAW, manager)
    assert not _null_byte_threats(result)
    assert result["is_threat"] is False
...
async def test_ldap_null_byte_finditer_overlap_text_is_detected(
    manager: SusPatternsManager,
) -> None:
    result = await _manager_detect("uid=a*)" + _NUL_RAW + "uid=b*)" + _NUL_RAW, manager)
...
async def test_ldap_null_byte_finditer_miss_text_detected_with_real_compiled(
    manager: SusPatternsManager,
) -> None:
    result = await _manager_detect("uid=a*)" + _NUL_RAW, manager)
    assert _null_byte_threats(result)
