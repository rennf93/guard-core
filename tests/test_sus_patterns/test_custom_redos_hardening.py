...
from guard_core.detection_engine._redos_literal_in_wildcard import (
...
@pytest.mark.redos_timing
async def test_overlapping_alternation_pattern_rejected_at_registration() -> None:
    ok = await SusPatternsManager.add_pattern(r"(b|b)*c", custom=True)
    assert ok is False
...
async def test_custom_pattern_routed_through_pool_path() -> None:
    ...
    ) -> Callable[[str], Awaitable[list[re.Match]]]:
...
async def test_built_in_pattern_routed_through_inline_safe_path() -> None:
    captured: list[bool] = []
    original = PatternCompiler.create_async_safe_finditer_matcher
...
async def test_built_in_detect_is_fast_and_non_blocking() -> None:
