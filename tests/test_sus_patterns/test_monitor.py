...
@pytest.mark.asyncio
async def test_check_anomalies_statistical_ignores_fast_execution() -> None:
    monitor = PerformanceMonitor(anomaly_threshold=2.0)
...
@pytest.mark.asyncio
async def test_cooldown_suppresses_burst_for_same_pattern() -> None:
    monitor = PerformanceMonitor(
...
@pytest.mark.asyncio
async def test_cooldown_does_not_suppress_a_different_pattern() -> None:
    monitor = PerformanceMonitor(
        slow_pattern_threshold=0.05, anomaly_emission_cooldown=60.0
    )
    agent_handler = MagicMock()
...
@pytest.mark.asyncio
async def test_cooldown_allows_emission_again_after_window_elapses() -> None:
    monitor = PerformanceMonitor(
        slow_pattern_threshold=0.05, anomaly_emission_cooldown=60.0
    )
    agent_handler = MagicMock()
...
@pytest.mark.asyncio
async def test_cooldown_state_does_not_survive_max_tracked_patterns_eviction() -> None:
    monitor = PerformanceMonitor(
...
@pytest.mark.asyncio
async def test_min_samples_for_anomaly_above_100_still_fires() -> None:
