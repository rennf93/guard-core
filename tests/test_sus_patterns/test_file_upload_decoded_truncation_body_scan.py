...
async def _manager_truncation_threats(
...
async def _manager_decoded_truncation_threats(
    manager: SusPatternsManager, body: str
) -> list[dict]:
    result = await manager.detect(body, "203.0.113.9", context="request_body")
    return [
...
async def test_nul_truncation_evasion_in_body_fires_file_upload(
    body: str,
) -> None:
    original_instance = SusPatternsManager._instance
    original_config = SusPatternsManager._config
...
@pytest.mark.parametrize("body", NUL_TRUNCATION_EVASION_BODIES)
async def test_nul_truncation_evasion_fires_in_enhanced_mode_via_raw_view(
    manager: SusPatternsManager, body: str
) -> None:
    result = await manager.detect(body, "203.0.113.9", context="request_body")
    assert result["detection_method"] == "enhanced"
    threats = await _manager_truncation_threats(manager, body)
    assert threats
    assert threats[0]["category"] == "file_upload"


async def test_bare_dangerous_extension_in_body_still_fires() -> None:
    original_instance = SusPatternsManager._instance
    original_config = SusPatternsManager._config
    SusPatternsManager._instance = None
    SusPatternsManager._config = None
    SusPatternsManager(SecurityConfig())
    try:
        categories = await _body_categories('filename="shell.php"')
        assert "file_upload" in categories
    finally:
        SusPatternsManager._instance = original_instance
        SusPatternsManager._config = original_config
...
async def test_trailing_dot_truncation_in_body_still_fires() -> None:
    original_instance = SusPatternsManager._instance
    original_config = SusPatternsManager._config
    SusPatternsManager._instance = None
    SusPatternsManager._config = None
...
async def test_double_extension_in_body_still_fires() -> None:
    original_instance = SusPatternsManager._instance
    original_config = SusPatternsManager._config
    SusPatternsManager._instance = None
    SusPatternsManager._config = None
    SusPatternsManager(SecurityConfig())
    try:
        categories = await _body_categories('filename="shell.php.jpg"')
        assert "file_upload" in categories
    finally:
        SusPatternsManager._instance = original_instance
        SusPatternsManager._config = original_config
...
async def test_benign_php_txt_filename_in_body_not_flagged(
    manager: SusPatternsManager,
) -> None:
    assert await _body_is_threat('filename="shell.php.txt"') is False
...
async def test_benign_photo_filename_in_body_not_flagged(
    manager: SusPatternsManager,
) -> None:
...
async def test_space_bridge_is_not_a_double_extension_vector() -> None:
    original_instance = SusPatternsManager._instance
    original_config = SusPatternsManager._config
    SusPatternsManager._instance = None
    SusPatternsManager._config = None
    ...
    try:
        ...
        categories = await _body_categories('filename="vacation photo.php .jpg"')
        assert "file_upload" not in categories
    finally:
        SusPatternsManager._instance = original_instance
        SusPatternsManager._config = original_config
...
async def test_control_char_bridge_is_a_double_extension_vector(body: str) -> None:
    original_instance = SusPatternsManager._instance
    original_config = SusPatternsManager._config
    SusPatternsManager._instance = None
    SusPatternsManager._config = None
...
async def test_decoded_truncation_pattern_registered_in_url_decoded_view_only(
    manager: SusPatternsManager,
) -> None:
    assert (
...
async def test_benign_filenames_do_not_fire_decoded_truncation_pattern(
    manager: SusPatternsManager, body: str
) -> None:
    threats = await _manager_decoded_truncation_threats(manager, body)
    assert not threats
...
async def test_benign_base64_blob_in_filename_does_not_fire_decoded_truncation_pattern(
    manager: SusPatternsManager,
) -> None:
...
async def test_benign_base64_decoded_blobs_do_not_fire_decoded_truncation_pattern(
    manager: SusPatternsManager, body: str
) -> None:
    threats = await _manager_decoded_truncation_threats(manager, body)
    assert not threats
...
async def test_double_encoded_semicolon_fires_file_upload_in_url_decoded_view(
    body: str,
) -> None:
    original_instance = SusPatternsManager._instance
    original_config = SusPatternsManager._config
...
async def test_single_encoded_semicolon_fires_file_upload_in_url_decoded_view(
    body: str,
) -> None:
    original_instance = SusPatternsManager._instance
    original_config = SusPatternsManager._config
...
async def test_encoded_trailing_dot_fires_file_upload_in_url_decoded_view(
    body: str,
) -> None:
    original_instance = SusPatternsManager._instance
    original_config = SusPatternsManager._config
...
async def test_decoded_truncation_manager_detects_semicolon_and_dot_bodies(
    manager: SusPatternsManager,
) -> None:
    assert (
        _FILE_UPLOAD_DECODED_TRUNCATION_RE in DETECTION_URL_DECODED_VIEW_PATTERN_SOURCES
