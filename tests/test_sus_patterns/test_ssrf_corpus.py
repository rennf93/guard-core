...
from guard_core.detection_engine import PatternCompiler
...
async def test_known_good_benign_targets_stay_unflagged(payload: str) -> None:
    result = await sus_patterns_handler.detect(
...
async def test_alternate_encoded_loopback_and_private_ip_flagged_as_ssrf(
    payload: str,
) -> None:
    result = await sus_patterns_handler.detect(
        content=payload, ip_address="203.0.113.9", context="request_body"
...
async def test_cloud_metadata_hostname_and_ip_flagged_as_ssrf(payload: str) -> None:
    result = await sus_patterns_handler.detect(
        content=payload, ip_address="203.0.113.9", context="request_body"
    )
    assert result["is_threat"] is True
...
async def test_public_and_malformed_addresses_not_flagged_as_ssrf(
    payload: str,
) -> None:
    result = await sus_patterns_handler.detect(
        content=payload, ip_address="198.51.100.4", context="request_body"
...
async def test_benign_url_corpus_not_flagged_as_ssrf(payload: str) -> None:
    result = await sus_patterns_handler.detect(
        content=payload, ip_address="198.51.100.4", context="request_body"
    )
    assert result["is_threat"] is False
...
@pytest.mark.asyncio
async def test_legacy_ipv4_not_blocked_yields_no_threat_legacy_state() -> None:
    pattern = re.compile(_LEGACY_IPV4_HOST_RE, re.IGNORECASE)
    threat, timed_out = await sus_patterns_handler._check_regex_pattern(
        pattern,
        "http://8.8.8.8/",
        "203.0.113.9",
        time.monotonic(),
...
@pytest.mark.asyncio
async def test_legacy_ipv4_not_blocked_yields_no_threat_enhanced_state() -> None:
    pattern = re.compile(_LEGACY_IPV4_HOST_RE, re.IGNORECASE)
