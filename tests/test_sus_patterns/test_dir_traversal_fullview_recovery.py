...
@pytest.mark.asyncio
async def test_etc_passwd_form_urlencoded_trailing_field_fires() -> None:
    await _assert_body_fires_dir_traversal("p=/etc/passwd&bar=1")


@pytest.mark.asyncio
async def test_etc_passwd_path_info_matrix_trailing_fires() -> None:
    await _assert_body_fires_dir_traversal("/etc/passwd;x=1")


@pytest.mark.asyncio
async def test_etc_passwd_json_trailing_field_fires() -> None:
    await _assert_body_fires_dir_traversal('{"p":"/etc/passwd&bar=1"}')


@pytest.mark.asyncio
async def test_etc_passwd_xml_trailing_sibling_fires() -> None:
    await _assert_body_fires_dir_traversal("<doc><p>/etc/passwd</p><x>1</x></doc>")


@pytest.mark.asyncio
async def test_etc_passwd_url_encoded_trailing_field_fires() -> None:
    await _assert_body_fires_dir_traversal("%2fetc%2fpasswd&bar=1")


@pytest.mark.asyncio
async def test_proc_environ_form_urlencoded_trailing_field_fires() -> None:
    await _assert_body_fires_dir_traversal("p=/proc/self/environ&x=1")


@pytest.mark.asyncio
async def test_windows_ini_form_urlencoded_trailing_field_fires() -> None:
    await _assert_body_fires_dir_traversal("p=boot.ini&x=1")


@pytest.mark.asyncio
async def test_var_log_form_urlencoded_trailing_field_fires() -> None:
    await _assert_body_fires_dir_traversal("p=var/log/auth.log&x=1")


@pytest.mark.asyncio
async def test_etc_passwd_prose_with_trailing_words_not_detected() -> None:
    await _assert_body_does_not_fire("the file was /etc/passwd and more")
