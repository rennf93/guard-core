...
async def test_bind_token_tautology_detected(request_obj: MockGuardRequest) -> None:
...
async def test_existing_tautology_controls_still_detected(
    request_obj: MockGuardRequest,
) -> None:
    result = await detect_penetration_attempt(request_obj, _CONFIG)
    assert result.is_threat is True
    assert "sqli" in result.threat_categories
...
async def test_bind_placeholder_without_tautology_shape_not_detected(
    request_obj: MockGuardRequest,
) -> None:
    result = await detect_penetration_attempt(request_obj, _CONFIG)
