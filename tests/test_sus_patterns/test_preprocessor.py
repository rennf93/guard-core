...
@pytest.mark.asyncio
async def test_truncate_attacks_exceed_max_length_breaks_loop() -> None:
...
@pytest.mark.asyncio
async def test_truncate_attacks_iterate_without_break() -> None:
    preprocessor = ContentPreprocessor(max_content_length=500)
...
@pytest.mark.asyncio
async def test_decode_reaches_max_iterations() -> None:
    preprocessor = ContentPreprocessor()
    nested = "%25%32%35%32%35%33%31" + "script"
    result = await preprocessor.decode_common_encodings(nested)
