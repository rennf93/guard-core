...
def test_initialize_redis_assigns_handler() -> None:
...
def test_ban_exact_ip_sends_event_when_agent_handler_present() -> None:
    manager = IPBanManager()
    manager.redis_handler = None
    agent = MagicMock()
    manager.agent_handler = agent

    manager.ban_ip("10.0.0.20", duration=60, reason="r")
...
def test_send_ban_event_logs_error_when_agent_raises(
    ...
    caplog.set_level(logging.ERROR, logger="guard_core.sync.handlers.ipban")
    manager.ban_ip("10.0.0.21", duration=60, reason="r")

    errors = [r for r in caplog.records if r.levelno == logging.ERROR]
    assert any("Failed to send ban event" in r.getMessage() for r in errors)


def test_unban_removes_local_entry_and_calls_redis_delete() -> None:
    manager = IPBanManager()
    redis = MagicMock()
    ...
    manager.unban_ip("10.0.0.30")

    assert "10.0.0.30" not in manager.banned_ips
    redis.delete.assert_called_once_with("banned_ips", "10.0.0.30")
    agent.send_event.assert_called_once()
    sent = agent.send_event.call_args.args[0]
    assert sent.action_taken == "unbanned"


def test_unban_when_ip_absent_and_no_redis_no_agent_is_noop() -> None:
    manager = IPBanManager()
    manager.redis_handler = None
    manager.agent_handler = None

    manager.unban_ip("10.0.0.31")

    assert "10.0.0.31" not in manager.banned_ips


def test_send_unban_event_logs_error_when_agent_raises(
    caplog: pytest.LogCaptureFixture,
) -> None:
    manager = IPBanManager()
    manager.redis_handler = None
    ...
    caplog.set_level(logging.ERROR, logger="guard_core.sync.handlers.ipban")
    manager.unban_ip("10.0.0.32")

    errors = [r for r in caplog.records if r.levelno == logging.ERROR]
    assert any("Failed to send unban event" in r.getMessage() for r in errors)


def test_is_ip_banned_clears_expired_local_entry_and_returns_false() -> None:
    manager = IPBanManager()
    manager.redis_handler = None
    manager.banned_ips["10.0.0.40"] = time.time() - 1

    assert manager.is_ip_banned("10.0.0.40") is False
    assert "10.0.0.40" not in manager.banned_ips


def test_is_ip_banned_uses_redis_when_local_miss() -> None:
    manager = IPBanManager()
    redis = MagicMock()
    redis.get_key.return_value = str(time.time() + 60)
    manager.redis_handler = redis

    assert manager.is_ip_banned("10.0.0.41") is True
    redis.get_key.assert_called_once_with("banned_ips", "10.0.0.41")


def test_check_redis_exact_returns_false_when_no_entry() -> None:
    manager = IPBanManager()
    redis = MagicMock()
    redis.get_key.return_value = None
    manager.redis_handler = redis

    assert manager.is_ip_banned("10.0.0.42") is False


def test_check_redis_exact_deletes_expired_entry_and_returns_false() -> None:
    manager = IPBanManager()
    redis = MagicMock()
    redis.get_key.return_value = str(time.time() - 10)
    manager.redis_handler = redis

    assert manager.is_ip_banned("10.0.0.43") is False
    redis.delete.assert_called_once_with("banned_ips", "10.0.0.43")


def test_reset_clears_local_state_and_redis_keys() -> None:
    manager = IPBanManager()
...
def test_reset_without_redis_just_clears_local() -> None:
    manager = IPBanManager()
    manager.redis_handler = None
    manager.banned_ips["x"] = time.time() + 60
