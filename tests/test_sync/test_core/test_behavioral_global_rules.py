...
def test_global_return_rules_process_without_correlation() -> None:
    ...
    for _ in range(3):
        processor.process_global_return_rules(request, response, "1.2.3.4", [rule])
...
def test_correlation_halves_threshold_when_ip_has_detections() -> None:
    processor, event_bus, _ = _build_processor(
...
def test_correlation_does_not_fire_when_ip_clean() -> None:
...
def test_correlation_halves_threshold_floor_at_one() -> None:
    processor, event_bus, _ = _build_processor(
...
def test_non_return_pattern_rules_are_ignored() -> None:
    processor, event_bus, _ = _build_processor()
...
def test_global_return_rules_use_middleware_tracker_without_decorator() -> None:
    config = SecurityConfig()
...
def test_global_return_rules_short_circuit_when_no_tracker() -> None:
...
def test_global_return_rules_handle_missing_middleware() -> None:
...
def test_global_return_rules_zero_count_ignored_for_correlation() -> None:
    processor, event_bus, _ = _build_processor(
...
def test_global_return_rules_apply_action_invoked() -> None:
    config = SecurityConfig()
    tracker = MagicMock(spec=BehaviorTracker)
    tracker.track_return_pattern = MagicMock(return_value=True)
    tracker.apply_action = MagicMock()
    event_bus = MagicMock()
    event_bus.send_middleware_event = MagicMock()
    middleware = MagicMock()
