...
def test_is_ip_in_blacklist_cidr_miss_then_match() -> None:
    assert (
...
def test_is_ip_in_whitelist_cidr_miss_then_match() -> None:
    assert (
...
def test_check_user_agent_skips_non_matching_pattern() -> None:
    rc = RouteConfig()
...
def test_check_user_agent_no_route_config() -> None:
    config = SecurityConfig(enable_redis=False)
    result = check_user_agent_allowed("Mozilla/5.0", None, config)
    assert result is True
