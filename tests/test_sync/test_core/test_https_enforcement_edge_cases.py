...
def test_check_passive_mode_no_redirect(
    ...
    result = https_check.check(request)
...
def test_handle_missing_referrer_passive_mode_unit() -> None:
    config = SecurityConfig()
    ...
    with patch(
        "guard_core.sync.core.checks.implementations.referrer.log_activity",
...
def test_handle_invalid_referrer_passive_mode_unit() -> None:
    config = SecurityConfig()
    config.passive_mode = True
...
def test_authentication_check_passive_mode_unit() -> None:
    config = SecurityConfig()
    config.passive_mode = True
    ...
    with patch(
        "guard_core.sync.core.checks.implementations.authentication.log_activity",
        return_value=MagicMock(),
    ):
...
def test_emergency_mode_check_no_client_ip_extracts_unit() -> None:
    config = SecurityConfig()
    ...
    middleware.create_error_response = MagicMock(return_value=Mock(status_code=503))
    ...
    with patch(
        "guard_core.sync.core.checks.implementations.emergency_mode.extract_client_ip",
        return_value="8.8.8.8",
    ):
        with patch(
            "guard_core.sync.core.checks.implementations.emergency_mode.log_activity",
            return_value=MagicMock(),
        ):
            result = check.check(request)
...
def test_emergency_mode_check_passive_mode_unit() -> None:
    config = SecurityConfig()
    config.passive_mode = True
    config.emergency_mode = True
...
def test_check_request_size_limit_passive_mode_unit() -> None:
    config = SecurityConfig()
    config.passive_mode = True
    ...
    with patch(
        "guard_core.sync.core.checks.implementations.request_size_content.log_activity",
        return_value=MagicMock(),
    ):
...
def test_check_content_type_allowed_passive_mode_unit() -> None:
    config = SecurityConfig()
    config.passive_mode = True
...
def test_required_headers_check_passive_mode_unit() -> None:
    config = SecurityConfig()
    config.passive_mode = True
    ...
    with patch(
        "guard_core.sync.core.checks.implementations.required_headers.log_activity",
        return_value=MagicMock(),
    ):
        result = check.check(request)
        assert result is None
...
def test_suspicious_activity_check_no_client_ip_unit() -> None:
    config = SecurityConfig()
...
def test_suspicious_activity_check_passive_mode_unit() -> None:
    config = SecurityConfig()
    config.passive_mode = True
    config.enable_penetration_detection = True
    ...
    with patch(
        "guard_core.sync.core.checks.implementations.suspicious_activity.get_cached_detection_result",
