...
from guard_core.sync.core.validation import validator as validator_module
...
def test_is_path_excluded_rejects_prefix_confusion_at_validator_level(
    validator: RequestValidator, mock_request: Any, mock_event_bus: Any
) -> None:
...
def test_is_path_excluded_rejects_traversal_escaping_subtree_at_validator_level(
    validator: RequestValidator, mock_request: Any, mock_event_bus: Any
) -> None:
    validator.context.config.exclude_paths = ["/static"]
...
def test_is_path_excluded_emits_event_once_across_many_requests(
    validator: RequestValidator, mock_request: Any, mock_event_bus: Any
) -> None:
    mock_request.url_path = "/health"

    results = [validator.is_path_excluded(mock_request) for _ in range(100)]

    assert results == [True] * 100
    mock_event_bus.send_middleware_event.assert_called_once()


def test_is_path_excluded_emits_separate_event_for_second_distinct_path(
    validator: RequestValidator, mock_request: Any, mock_event_bus: Any
) -> None:
    mock_request.url_path = "/health"
    validator.is_path_excluded(mock_request)
...
def test_is_path_excluded_emits_again_after_ttl_expires(
    validator: RequestValidator, mock_request: Any, mock_event_bus: Any
) -> None:
    mock_request.url_path = "/health"
...
def test_is_path_excluded_decision_unaffected_by_cache_hit(
    validator: RequestValidator, mock_request: Any, mock_event_bus: Any
) -> None:
    mock_request.url_path = "/health"

    first = validator.is_path_excluded(mock_request)
    second = validator.is_path_excluded(mock_request)
...
def test_is_path_excluded_cached_entry_does_not_exclude_unrelated_path(
    validator: RequestValidator, mock_request: Any, mock_event_bus: Any
) -> None:
    mock_request.url_path = "/health"
    validator.is_path_excluded(mock_request)
...
def test_is_path_excluded_unresolvable_request_path_is_not_excluded(
    validator: RequestValidator, mock_request: Any, mock_event_bus: Any
) -> None:
...
def test_is_path_excluded_throttle_keyed_on_normalized_path_not_raw(
    validator: RequestValidator, mock_request: Any, mock_event_bus: Any
) -> None:
...
def test_is_path_excluded_reflects_exclude_paths_mutated_at_runtime() -> None:
    ...
    assert validator.is_path_excluded(request) is False

    config.exclude_paths = ["/new"]

    assert validator.is_path_excluded(request) is True


def test_is_path_excluded_normalizes_exclude_list_once_until_content_changes(
    ...
    for _ in range(5):
        validator.is_path_excluded(request)
    assert call_count == 1
...
def test_is_path_excluded_reflects_exclude_paths_appended_in_place() -> None:
...
def test_is_path_excluded_reflects_exclude_paths_shrunk_in_place() -> None:
...
def test_is_path_excluded_reflects_size_preserving_in_place_replace() -> None:
