...
def test_execute_all_checks_pass(mock_middleware: Mock, mock_request: Mock) -> None:
...
def test_execute_first_check_blocks(mock_middleware: Mock, mock_request: Mock) -> None:
...
def test_execute_second_check_blocks(mock_middleware: Mock, mock_request: Mock) -> None:
    check1 = MockCheck(mock_middleware, "check1", should_block=False)
...
def test_run_post_response_passes_through_when_no_replacement(
    mock_middleware: Mock, mock_request: Mock
) -> None:
    ...
    result = pipeline.run_post_response(mock_request, original)

    assert result is original


def test_run_post_response_uses_replacement(
    mock_middleware: Mock, mock_request: Mock
) -> None:
    original = Mock(status_code=200)
...
def test_run_post_response_chains_replacements(
    mock_middleware: Mock, mock_request: Mock
) -> None:
    original = Mock(status_code=200)
...
def test_run_post_response_swallows_check_exceptions(
    mock_middleware: Mock, mock_request: Mock
) -> None:
    original = Mock(status_code=200)
    checks: list[SecurityCheck] = [
...
def test_default_post_response_is_noop(
    mock_middleware: Mock, mock_request: Mock
) -> None:
    check = MockCheck(mock_middleware, "legacy")

    result = check.post_response(mock_request, Mock(status_code=200))
