...
def test_init(behavioral_context: Mock) -> None:
    processor = BehavioralProcessor(behavioral_context)
    assert processor.context == behavioral_context


def test_process_usage_rules_no_decorator(processor: Mock, mock_request: Mock) -> None:
...
def test_process_usage_rules_threshold_exceeded(
    ...
    processor.context.guard_decorator.behavior_tracker.track_endpoint_usage = MagicMock(
        return_value=True
    )
...
def test_process_return_rules_pattern_detected(
    ...
    processor.context.guard_decorator.behavior_tracker.track_return_pattern = MagicMock(
        return_value=True
    )
...
def test_process_usage_rules_skips_return_pattern_rules(
...
def test_process_usage_rules_uses_request_state_decorator_tracker(
    mock_request: Mock,
) -> None:
    request_tracker = Mock()
    request_tracker.track_endpoint_usage = MagicMock(return_value=False)
    ...
    request_tracker.track_endpoint_usage.assert_called_once()
