...
from guard_core.sync.detection_engine._redos_parse_slots import _regex_parser
...
def test_repeat_discovery_inside_atomic_group(monkeypatch: pytest.MonkeyPatch) -> None:
    from guard_core.sync.detection_engine._redos_prefix_history import _contains_repeat
...
def test_group_probe_candidate_expansion_has_a_budget(
    ...
    monkeypatch.setattr(
        "guard_core.sync.detection_engine._redos_prefixed_probe._GROUP_PREFIX_CANDIDATE_LIMIT",
        4,
    )
