...
def test_defect_5_control_bare_glued_search_whoami_stays_detected() -> None:
    assert "cmd_injection" in _detected_categories("search`whoami`")


def test_adversarial_denylist_token_detected_with_keyword_after_payload() -> None:
    payload = "curl`whoami` ORDER BY name"
    assert "cmd_injection" in _detected_categories(payload)


def test_adversarial_denylist_token_detected_with_nearby_keyword() -> None:
