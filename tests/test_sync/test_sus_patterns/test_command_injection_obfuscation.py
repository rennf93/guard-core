...
def test_brace_expansion_command_is_detected(
...
def test_quote_splice_dangerous_command_is_detected(
    manager: SusPatternsManager,
) -> None:
    result = manager.detect(
        "c'a't config.ini",
...
def test_glob_wildcard_dangerous_command_is_detected(
    manager: SusPatternsManager,
) -> None:
    result = manager.detect(
        "c?t config.ini",
...
def test_glob_wildcard_non_dangerous_token_is_not_flagged(
    manager: SusPatternsManager,
) -> None:
    result = manager.detect(
        "fo?bar",
