...
def test_brace_expansion_at_content_start_is_detected(
...
def test_brace_expansion_with_non_command_first_item_is_detected(
    manager: SusPatternsManager,
) -> None:
    result = manager.detect(
        "{banana,config.ini}",
...
def test_brace_expansion_after_operator_boundary_is_detected(
    manager: SusPatternsManager,
) -> None:
    result = manager.detect(
        "id=5;{rm,-rf,/}",
...
def test_brace_expansion_embedded_in_prose_is_not_flagged(
    manager: SusPatternsManager,
) -> None:
    result = manager.detect(
...
def test_json_object_shape_is_not_flagged_as_brace_expansion(
    manager: SusPatternsManager,
) -> None:
    result = manager.detect(
...
def test_quote_splice_uniform_single_char_run_is_detected(
    manager: SusPatternsManager,
) -> None:
    result = manager.detect(
        "c'a't config.ini",
...
def test_quote_splice_non_command_uniform_run_is_detected(
    manager: SusPatternsManager,
) -> None:
    result = manager.detect(
...
def test_glob_wildcard_after_operator_boundary_is_detected(
    manager: SusPatternsManager,
) -> None:
    result = manager.detect(
        "|c?t config.ini",
...
def test_python_getattr_indirection_is_detected(
    manager: SusPatternsManager,
) -> None:
    result = manager.detect(
...
def test_quote_splice_non_dangerous_token_is_not_flagged(
    manager: SusPatternsManager,
) -> None:
    result = manager.detect(
        "foo'ba'r",
...
def test_glob_wildcard_without_operator_boundary_is_not_flagged(
    manager: SusPatternsManager,
) -> None:
    result = manager.detect(
        "fo?bar",
...
def test_glob_wildcard_bare_dangerous_shape_without_boundary_is_not_flagged(
    manager: SusPatternsManager,
) -> None:
    result = manager.detect(
        "c?t config.ini",
