...
def test_fetch_azure_ip_ranges_url_in_plain_text(
...
def test_fetch_azure_ip_ranges_ignores_non_servicetags_json(
    mock_aiohttp_session: MagicMock,
) -> None:
    mock_html_resp = _mock_aiohttp_response(
        text_data='var m = "https://download.microsoft.com/download/manifests/index.json";'
    )
    mock_aiohttp_session.get = MagicMock(return_value=mock_html_resp)

    result = fetch_azure_ip_ranges()
    assert result == set()


def test_fetch_azure_ip_ranges_preserves_query_string(
    mock_aiohttp_session: MagicMock,
) -> None:
    mock_html_resp = _mock_aiohttp_response(
...
def test_fetch_azure_ip_ranges_prefers_servicetags_link_over_unrelated_json(
    mock_aiohttp_session: MagicMock,
) -> None:
    mock_html_resp = _mock_aiohttp_response(
        text_data="""
...
def test_fetch_azure_ip_ranges_ignores_stale_url_mentioned_before_download_link(
    mock_aiohttp_session: MagicMock,
) -> None:
    stale_url = (
...
def test_fetch_azure_ip_ranges_falls_back_to_newest_when_no_failover_link(
    mock_aiohttp_session: MagicMock,
) -> None:
    older_url = (
        "https://download.microsoft.com/download/7/1/D/ServiceTags_Public_20230130.json"
...
def test_download_azure_service_tags_defaults_to_a_full_budget_with_no_deadline(
    mock_aiohttp_session: MagicMock,
) -> None:
    mock_json_resp = _mock_aiohttp_response(
        json_data={"values": [{"properties": {"addressPrefixes": ["10.0.0.0/8"]}}]}
    ...
    data = _download_azure_service_tags("https://download.microsoft.com/valid.json")
...
def test_fetch_azure_ip_ranges_fails_fast_when_deadline_already_exhausted(
    mock_aiohttp_session: MagicMock,
) -> None:
    with patch(
        "guard_core.sync.handlers.cloud_handler.time.monotonic",
...
def test_fetch_azure_ip_ranges_falls_back_when_failover_link_is_untrusted(
    mock_aiohttp_session: MagicMock,
) -> None:
    legit_url = (
        "https://download.microsoft.com/download/7/1/D/ServiceTags_Public_20250116.json"
...
def test_extract_newest_service_tags_url_warns_when_no_candidate_has_a_parseable_date(
    ...
    with caplog.at_level(logging.WARNING, logger="guard_core.sync.handlers.cloud"):
...
def test_fetch_azure_ip_ranges_bad_status_is_not_retried(
    mock_aiohttp_session: MagicMock,
) -> None:
    mock_html_resp = _mock_aiohttp_response(
    ...
    with patch("guard_core.sync.handlers.cloud_handler.time.sleep") as sleep_mock:
        result = fetch_azure_ip_ranges()
    assert result == set()
...
def test_fetch_azure_ip_ranges_bad_json_body_is_not_retried(
    mock_aiohttp_session: MagicMock,
) -> None:
    mock_html_resp = _mock_aiohttp_response(
        text_data='<a href="https://download.microsoft.com/valid.json">'
    ...
    mock_download_resp = MagicMock()
    mock_download_resp.raise_for_status = MagicMock()
    mock_download_resp.json = MagicMock(side_effect=ValueError("Expecting value"))
    mock_aiohttp_session.get = MagicMock(
        side_effect=[mock_html_resp, mock_download_resp]
    )
    with patch("guard_core.sync.handlers.cloud_handler.time.sleep") as sleep_mock:
...
def test_fetch_azure_ip_ranges_gives_up_after_max_attempts(
    mock_aiohttp_session: MagicMock,
) -> None:
    mock_html_resp = _mock_aiohttp_response(
        text_data='<a href="https://download.microsoft.com/valid.json">'
...
def test_fetch_azure_ip_ranges_stops_retrying_past_elapsed_bound(
    mock_aiohttp_session: MagicMock,
) -> None:
    mock_html_resp = _mock_aiohttp_response(
        text_data='<a href="https://download.microsoft.com/valid.json">'
    ...
    with (
        patch("guard_core.sync.handlers.cloud_handler.time.sleep") as sleep_mock,
        patch(
            "guard_core.sync.handlers.cloud_handler.time.monotonic",
...
def test_fetch_azure_ip_ranges_checks_elapsed_before_each_attempt(
    mock_aiohttp_session: MagicMock,
) -> None:
    mock_html_resp = _mock_aiohttp_response(
        text_data='<a href="https://download.microsoft.com/valid.json">'
...
def test_is_cloud_ip_warns_on_empty_ranges_without_changing_return_value(
    ...
    with caplog.at_level("WARNING", logger="guard_core.sync.handlers.cloud"):
...
def test_is_cloud_ip_empty_ranges_warning_is_throttled(
    ...
    with (
        ...
        caplog.at_level("WARNING", logger="guard_core.sync.handlers.cloud"),
    ):
        for _ in range(5):
